...
// If p_skin is non-null, ARRAY_BONES/ARRAY_WEIGHTS are also filled per corner (Godot's standard
// 4-wide layout): each corner's control point is looked up in p_skin->deformer->vertices[]
// (already weight-sorted descending by ufbx), the first 4 weights are resolved through
// p_skin->cluster_to_bone (dropping any that resolve to -1) and renormalized to sum to 1.
// Deliberately not widened to Godot's max of 8 (via Mesh::ARRAY_FLAG_USE_8_BONE_WEIGHTS) even
// though some corrective-bone-heavy rigs blend a vertex across 5+ bones and lose a little
// accuracy at 4 (measured ~6.5% of total weight mass dropped in the worst case on one test rig)
// - the GL Compatibility renderer doesn't correctly interpret the 8-wide vertex format (its
// skinning shader is hardcoded for 4-per-vertex), so surfaces come out badly scrambled, not just
// less accurate, on any project using that render method. 4 works correctly everywhere.
